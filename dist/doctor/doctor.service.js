...
let DoctorService = class DoctorService {
    ...
    create(createDto) {
        const doctor = this.doctorRepo.create(createDto);
        return this.doctorRepo.save(doctor);
    }
    findAll(query) {
        const where = {};
        if (query?.specialization)
            where.specialization = (0, typeorm_2.Like)(`%${query.specialization}%`);
        if (query?.location)
            where.location = (0, typeorm_2.Like)(`%${query.location}%`);
        return this.doctorRepo.find({ where });
    }
    async findOne(id) {
        const d = await this.doctorRepo.findOne({ where: { id } });
        if (!d)
